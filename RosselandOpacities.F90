! 
! *** MAIN program 'Opacity' is at the end of this file ***
!
!
!  This module uses the opacities from Semenov 2003
!  The following comments are from the original
!  subroutines, downloaded from
!
!  http://www.mpia-hd.mpg.de/homes/henning/Dust_opacities/Opacities/opacities.html
!
!     .........................................................................
...
!     .  following:
!     .  1) "xxx" means mineralogical composition of the silicates, i.e., 
...
! Close input file:
      close(07)
! For what kind of opacity calculations will be performed:
         ross = .true.
         print*,'----------------------------------------------------'
         write(*,*) 'initializing dust module'
         if (ross) then
            write(*,*) 'Using Rosseland mean opacities '        
         else
            write(*,*) 'Using Planck mean opacities '        
         end if
         write(*,*) 'according to Semenov 2003'
         write(*,*) ''
         write(*,*) 'These opacities are'
         write(*,*) 'temperature-dependent and assume'
         write(*,*) 'perfect black-body radiation.'
         print*,'----------------------------------------------------'
...
!
!-----------------------------------------------------------------------------
!   wrapper routine for the Semenjov routines
!-----------------------------------------------------------------------------
!
subroutine getOpacity(temp,rho,opac)
   ! 
   implicit none
   !
   real*8, intent(in)    :: temp,rho
   real*8, intent(out)   :: opac 
   ...
   ! 
   CALL cop(eD,eG,rho,temp,opac)
   !
   !opac = gasToDustRatio*opac
   !
...
! If 'YP' outside the range of 'Y' then print an error message and stop:
      IF (ip.eq.0) THEN
          print *,'EINT: YP outside the range of Y. (XP,YP)=', XP, YP
       stop
      END IF
...
!!******************************************************
  ...
  integer :: nArg, cptArg
  character(len=20) :: nameArg
  
  real :: temp, dens, kappa, opac

  ! parse the arguments
  nArg = command_argument_count()
  if (narg .ne. 2) then
      print *, 'USAGE: semenov [TEMPERATURE] [DENSITY]'
      return
  else
      do cptArg = 1, nArg
          call get_command_argument(cptArg, nameArg)
          if (cptArg .eq. 1) read (nameArg,*), temp
          if (cptArg .eq. 2) read (nameArg,*), dens
      end do
  endif

  ! return the dust+gas opacity from the Semenov table
  ! in the density range 2e-18 to 2e-7 g/cm^3 and
  ! in the tmperature range 5 to 10^4 K
  call getOpacity(temp, dens, kappa)

  ! CF 2017, to avoid zero opacity, we set it to a small value,
  ! e.g., 0.1 cm^2/g (see e.g., Krumholz et al. 2007)
  if (kappa .lt. 1e-10) then
     !write(*,*) 'WARNING: small kappa! (kappa, dens, temp) = ', kappa, dens, temp
     !write(*,*) '...setting it to a small constant.'
     kappa = 1e-10
  endif
  ...
  print *, 'input density     = ', dens
  print *, 'output opacity    = ', opac
  print *, 'output kappa      = ', kappa
